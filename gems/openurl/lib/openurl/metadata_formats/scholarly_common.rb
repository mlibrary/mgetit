...
module OpenURL
  class ScholarlyCommon < ContextObjectEntity
    ...
    def method_missing(metadata, value = nil)
      ...
      raise ArgumentError, "#{meta} is not a valid #{self.class} metadata field." unless (@author_keys + @metadata_keys).index(meta)
    ...
    def import_xml_metadata(node)
      ...
      if mbv
        mbv.to_a.each do |m|
          set_metadata(m.name, m.get_text.value) if m.has_text?
          if m.has_elements?
            m.to_a.each do |md|
              set_metadata(md.name, md.get_text.value) if md.has_text?
            end
          end
        end
        auth_num = 0
        REXML::XPath.each(mbv, "fmt:authors/fmt:author | fmt:authors/fmt:au | fmt:authors/fmt:aucorp", {"fmt" => @xml_ns}) do |author|
          ...
          if author.name == "author"
            author.elements.each do |auth_elem|
              ...
              set_metadata(auth_elem.name, auth_elem.get_text.value) if auth_num == 0
            end
          elsif /^au$|^aucorp$/.match?(author.name)
            ...
            set_metadata(author.name, author.get_text.value) if auth_num == 0
          end
  ...
  class Author
    ...
    def empty?
      instance_variables.each do |ivar|
        return false if instance_variable_get(ivar)
      end
      true
    end
  end
