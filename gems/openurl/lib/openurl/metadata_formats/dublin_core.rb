  ...
  class DublinCore < ContextObjectEntity
      ...
      if mbv
        mbv.to_a.each do |m|
          m.children.each do |value|
            set_metadata(m.name, CGI.unescapeHTML(value.to_s))
          end
        end
      end
    end
    ...
    def import_dc(dc)
      ...
      doc.root.elements.each do |elem|
        set_metadata(elem.name, CGI.unescapeHTML(elem.children.to_s)) unless elem.children.empty?
      end
    end
