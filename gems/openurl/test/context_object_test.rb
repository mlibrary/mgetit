...
class ContextObjectTest < Test::Unit::TestCase
  ...
  def test_marc
    ...
    ctx = OpenURL::ContextObject.new_from_xml(xml)
    test_marc_values(ctx)
    ctx2 = OpenURL::ContextObject.new_from_xml(ctx.xml)
    test_marc_values(ctx2)
  end
  ...
  def test_kev_hybrid_values(ctx)
    assert(ctx.referrer.identifiers.index("info:sid/myid.com:mydb").is_a?(Integer))
    assert(ctx.referent.identifiers.index("info:doi/10.1126/science.275.5304.1320").is_a?(Integer))
    assert(ctx.referent.identifiers.index("info:pmid/9036860").is_a?(Integer))
  ...
  def test_kev_01_values(ctx)
    assert(ctx.referrer.identifiers.index("info:sid/myid:mydb").is_a?(Integer))
    assert(ctx.referent.identifiers.index("info:doi/10.1126/science.275.5304.1320").is_a?(Integer))
    assert(ctx.referent.identifiers.index("info:pmid/9036860").is_a?(Integer))
    assert_match(/info:doi\/10\.1126\/science\.275\.5304\.1320|info:pmid\/9036860/, ctx.referent.identifier)
    assert_equal(ctx.referent.format, "journal")
  ...
  def test_xml_values(ctx)
    ...
    assert(ctx.referrer.identifiers.index("info:sid/metalib.com:PUBMED").is_a?(Integer))
    assert(ctx.referent.identifiers.index("info:doi/10.1364/OL.29.000017").is_a?(Integer))
    assert(ctx.referent.identifiers.index("info:pmid/14719646").is_a?(Integer))
    assert_match(/info:doi\/10\.1364\/OL\.29\.000017|info:pmid\/14719646/, ctx.referent.identifier)
    assert(ctx.referringEntity.identifiers.index("info:doi/10.1063/1.1968421").is_a?(Integer))
    assert_equal(ctx.referent.format, "journal")
