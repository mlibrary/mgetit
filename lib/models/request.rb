...
class Request < ActiveRecord::Base
  ...
  def self.find_or_create(params, session, a_rails_request, options = {})
    ...
    co_params = context_object_params(a_rails_request)
    ...
    param_fingerprint = co_params_fingerprint(fingerprintable_params)

    client_ip = params["req.ip"] || (a_rails_request.respond_to?(:remote_ip) && a_rails_request.remote_ip)
    ...
    unless req || options[:allow_create] == false
      ...
      req = create_new_request!(params: params, session: session, rails_request: a_rails_request, contextobj_fingerprint: param_fingerprint, context_object: context_object)
    end
    req
  end
  ...
  def self.context_object_params(a_rails_request)
    ...
    co_params.merge!(CGI.parse(a_rails_request.raw_post)) if a_rails_request.respond_to?(:raw_post) && a_rails_request.raw_post
    ...
    if co_params["id"]
      co_params["id"].each do |id|
        co_params["id"].delete(id) if /^\d+$/.match?(id)
      end
    end
  ...
  def dispatched(service, status, exception = nil)
    ActiveRecord::Base.connection_pool.with_connection do
      ...
      ds ||= new_dispatch_object!(service, status)
  ...
  def can_dispatch?(service)
    ds = dispatched_services.where(service_id: service.service_id).first

    ds.nil? || (ds.status == DispatchedService::Queued) || (ds.status == DispatchedService::FailedTemporary)
  end
  ...
  def register_in_progress(service)
    ActiveRecord::Base.connection_pool.with_connection do
      ...
      if ds
        ...
        updated_count = dispatched_services.where(id: ds.id,
        ...
        ds = new_dispatch_object!(service, DispatchedService::InProgress)
        ds.save!
        return true
      end
    end
  ...
  def add_service_response(response_data)
    raise ArgumentError.new("missing required `:service` key") unless response_data[:service]
    raise ArgumentError.new("missing required `:service_type_value` key") unless response_data[:service_type_value]

    svc_resp = nil
    ActiveRecord::Base.connection_pool.with_connection do
      svc_resp = service_responses.build
  ...
  def any_services_in_progress?
    services_in_progress.length > 0
  end

  def to_context_object
    # Mostly just the referent
    context_object = referent.to_context_object

    # But a few more things
    context_object.referrer.add_identifier(referrer_id) if referrer_id

    context_object.requestor.set_metadata("ip", client_ip_addr) if client_ip_addr
  ...
  def get_service_type(svc_type, options = {})
    responses = if options[:refresh]
      ActiveRecord::Base.connection_pool.with_connection do
        service_responses.where(["service_type_value_name = ?", svc_type]).to_a
      end
    else
      # find on an assoc will go to db, unless we convert it to a plain
      # old array first.
      service_responses.to_a.find_all { |response|
        response.service_type_value == svc_type
      }
    end
  ...
  def new_dispatch_object!(service, status)
    service_id = if service.is_a?(Service)
      service.service_id
    else
      service.to_s
    end
  ...
  def dispatch_objects_with(options = {})
    ...
    dispatched_services.to_a.find_all do |ds|
  ...
  def self.create_new_request!(args)
    ...
    # Create the Request
    req = Request.new
    req.session_id = a_rails_request.session[:session_id]
    ...
    # Save client ip
    req.client_ip_addr = params["req.ip"] || (a_rails_request.respond_to?(:remote_ip) && a_rails_request.remote_ip)
    req.client_ip_is_simulated = true if req.client_ip_addr != (a_rails_request.respond_to?(:remote_ip) && a_rails_request.remote_ip)
  ...
  def find_dispatch_object(service)
    dispatched_services.where(service_id: service.service_id).first
  end
  ...
  def self.co_params_fingerprint(params)
    ...
    params.each do |pair|
      ...
      if excluded_keys.find { |exc_key| exc_key === pair[0] }
        params.delete(pair)
      elsif pair[1] && pair[1].respond_to?(:sort!)
        pair[1].sort!
      end
    end
