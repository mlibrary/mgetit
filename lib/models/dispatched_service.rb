class DispatchedService < ActiveRecord::Base
  ...
  # that dispatched.
  def service
    @service ||= ServiceStore.instantiate_service!(service_id, request)
  end
  ...
  def can_generate_service_types
    if ServiceStore.service_definition_for(service_id)
  ...
  def status=(a_status)
    a_status = FailedTemporary if a_status.is_a?(FalseClass)
    a_status = Successful if a_status.is_a?(TrueClass)
  ...
  def store_exception(a_exc)
    return if a_exc.nil? || !exception_info.nil?
  ...
  def failed?
    (status == FailedTemporary) || (status == FailedFatal)
  end
end
